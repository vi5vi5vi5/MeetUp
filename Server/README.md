...
| `tools/` | Скрипты: локальная сборка/запуск (`build.ps1` / `run.ps1`) и деплой в Docker (`update.sh` / `update.ps1`) | [`tools/README.md`](tools/README.md) |
...
Пути к Qt зашиты в `tools\build.ps1` — поправь, если Qt лежит в другом месте.
...
1. Запусти сервер (локально — `tools\run.ps1`, на сервере — `docker compose up -d --build`).
