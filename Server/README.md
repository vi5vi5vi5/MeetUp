...
Самый короткий путь без установки Qt/MinGW — Docker. Qt6 в образе ставится
из пакетов Debian, поэтому нужен только `docker`.

```bash
docker build -t meetup-server .
docker run -d --name meetup-server --restart unless-stopped \
  -p 80:80 -p 9000:9000 meetup-server
```

Затем открой **http://localhost**. Порт `9000` пробрасывается один-в-один:
веб-клиент жёстко коннектится на `ws://<hostname>:9000`, менять маппинг нельзя.

Обновление с GitHub и пересборка — одной командой (см. [`tools/`](tools/README.md)):

```bash
./tools/update.sh --force                                  # Linux / macOS / Git Bash
powershell -ExecutionPolicy Bypass -File tools\update.ps1 -Force  # Windows
```
