...
Column {
    ...
    Field {
        ...
        // Подпись под выбором меняется вместе с ним: у кодеков разная цена, и
        // сказать о ней надо в ту секунду, когда человек выбирает. Формула
        // одна на всех: когда уместен — чем платим.
        readonly property var hints: ({
            "auto": "H.264, при неудаче VP8. Меняется на лету — собеседники переключатся сами.",
            "h264": "Дёшев по процессору, часто считается видеокартой. Разумный выбор для ноутбука на батарее.",
            "vp8":  "Открывается там, где H.264 нет. При том же канале картинка чуть хуже.",
            "vp9":  "Та же картинка при меньшем канале — выручает на слабом интернете. Платим процессором.",
            "av1":  "Сжимал бы ещё лучше, но пока выключен — приём готовится."
        })
        hint: hints[AV.camCodec] || ""
        SegmentedControl {
            width: parent.width
            current: AV.camCodec
