...
public:
    ...
    // Полоса потеряна: кодировщик завис внутри драйвера, движок собрал полосу
    // заново на чистом потоке, а этот воркер остался при зависшем вызове. Если
    // драйвер однажды всё-таки вернётся, наружу не должно уйти НИЧЕГО: пакеты
    // старого кодировщика попали бы в середину чужого уже потока кадров, и
    // получатель разбирал бы их поверх несовместимого состояния.
    // Зовётся с GUI-потока, читается на своём — потому и атомик.
    void abandon() { m_abandoned.store(true, std::memory_order_release); }
...
private:
    ...
    std::atomic<bool> m_abandoned{ false };   // см. abandon()
