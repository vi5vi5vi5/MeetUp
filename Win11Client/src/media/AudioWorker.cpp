...
void AudioWorker::onFrame(const QByteArray& d) {
    ...
    //
    // Вероятности речи здесь взять негде — звук приезжает кодированным, а
    // гонять каждого участника через сеть RNNoise значило бы платить
    // процентом ядра за человека. Поэтому громкость, но относительно ЕГО
    // шумовой полки: абсолютный порог не знает ни как громко говорит этот
    // конкретный собеседник, ни какой у него фон, а участники все разные.
    if (!isScreen) {
        ...
        // Кадр в пару байт — это Opus, ужавший молчание (DTX): вердикт самого
        // отправителя о собственном сигнале, и он вернее любого измерения на
        // нашей стороне. Полку при этом кормим дальше — распакованный кадр
        // несёт комфортный шум, то есть ровно уровень его комнаты.
        const bool dtx = f.payload.size() <= 2;
        const bool speech = n && p.gate.feed(qSqrt(sum / n)) && !dtx;
        if (dtx) p.gate.reset();
        // Окно подсветки — 450 мс, поэтому дёргать GUI чаще, чем раз в 150 мс,
        // смысла нет (а кадров приходит полсотни в секунду).
        if (speech && now - p.lastSpokeAt >= 150) {
            p.lastSpokeAt = now;
