...
// Кого и в каком порядке пробуем открыть.
//
...
//
// Отдельной функцией, а не внутри open(), потому что этот же ответ нужен, не
// открывая ничего: по нему видно, совпадает ли выбор человека с тем, что уже
// работает (см. firstChoice).
static QList<VideoEncoder::Candidate> buildOrder(bool screen, int step,
                                                 const QString& preferred) {
    QList<VideoEncoder::Candidate> order;
    const auto add = [&order](const VideoEncoder::Candidate& c) {
        for (const VideoEncoder::Candidate& x : order)
            if (qstrcmp(x.name, c.name) == 0) return;
        order.append(c);
    };
    ...
    if (step <= 0)
        if (const VideoEncoder::Candidate* c = findCandidate(preferred)) add(*c);
...
QString VideoEncoder::firstChoice(bool screen, int step, const QString& preferred) {
    const QList<Candidate> order = buildOrder(screen, step, preferred);
    return order.isEmpty() ? QString() : QString::fromLatin1(order.first().name);
}
...
bool VideoEncoder::open(int width, int height, int fps, int bitrate,
    ...
    for (const Candidate& c : buildOrder(screen, step, preferred)) {
