...
public:
    ...
    // Кадр встаёт в очередь. Зовётся НА ПОТОКЕ ТРАНСПОРТА, до того как кадр
    // ляжет в очередь событий: очередь Qt ничем не ограничена, и разобрать её
    // задним числом нельзя — но можно знать, сколько в ней лежит (queued), и
    // объявить всё лежащее мусором (flush).
    //
    // Возвращает номер поколения, с которым кадр надо отдать в onFrame:
    // сброс увеличивает его, и всё, что уже лежит в очереди, воркер узнаёт по
    // старому номеру и молча выбросит, не декодируя.
    //
    // Выключателя у очереди нет намеренно. Он был: «без буфера» принимало кадр
    // только при разобранном предыдущем и роняло остальные на месте. Картинка
    // от этого рвалась всегда, даже когда в очереди набирался один кадр:
    // каждый выброшенный кадр рвёт поток и стоит паузы до опорного. Растущее
    // отставание лечит авто-сброс (setAutoResetMs) — очередь копится, но не
    // дольше порога.
    quint32 offer();
    ...
    // Порог авто-сброса, мс; 0 — выключен. Отставанием считается время, которое
    // кадр пролежал в очереди до декодера (см. onFrame).
    void setAutoResetMs(int ms) { m_autoResetMs.store(ms, std::memory_order_relaxed); }
    ...
    // Наибольшее отставание с прошлого вызова, мс. Читать и обнулять — как
    // takeRxBytes у транспорта: раз в секунду, для «Диагностики».
    int takeLagPeakMs() { return m_lagPeakMs.exchange(0, std::memory_order_relaxed); }
...
public slots:
    ...
    // arrivalMs — когда транспорт его принял (мс эпохи): по разнице с «сейчас»
    // считается отставание.
    void onFrame(const QByteArray& frame, quint32 generation, qint64 arrivalMs);
...
signals:
    ...
    // Очередь выброшена авто-сбросом: кадры лежали в ней дольше порога.
    // lagMs — сколько пролежал тот, на котором это заметили. Движок по этому
    // сигналу чистит придержанное под звук и говорит человеку.
    void autoReset(int lagMs);
...
private:
    ...
    std::atomic<int> m_autoResetMs{ 0 };
    std::atomic<int> m_lagPeakMs{ 0 };
    quint32 m_seenGeneration = 0;      // поколение последнего разобранного кадра
    qint64 m_lastAutoResetMs = 0;      // когда сбрасывали в последний раз
};
