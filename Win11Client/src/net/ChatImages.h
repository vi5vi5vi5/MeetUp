...
public:
    ...
    //
    // size, если он нужен, заполняется размером из ЗАГОЛОВКА jpeg — без
    // полного декодирования. Размер нужен ленте, чтобы отвести картинке место
    // сразу, не дожидаясь загрузки.
    QString put(const QByteArray& jpeg, QSize* size = nullptr);
