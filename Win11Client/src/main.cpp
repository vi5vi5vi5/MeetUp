...
int main(int argc, char *argv[])
{
    ...
    // Проверка работоспособности для установщика обновления (см.
    // UpdateChecker::installAndRestart). Дойти сюда — уже содержательный
    // ответ: значит загрузились все библиотеки, на которые ссылается exe, и
    // поднялся платформенный плагин Qt. Ровно это и ломается, когда пакет
    // обновления собран неполным, и ровно поэтому проверять надо ДО того, как
    // старая, работающая версия будет затёрта.
    //
    // Дальше не идём намеренно: загрузка QML построила бы окно и мигнула им
    // человеку, а отказы такого рода ловятся уже на этапе сборки.
    for (int i = 1; i < argc; ++i)
        if (qstrcmp(argv[i], "--selftest") == 0) return 0;
